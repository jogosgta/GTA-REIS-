...
class Player {
  ...
  constructor(scene: Scene) {
    ...
    this.body = box(scene, "player-body", { width: 0.72, height: 1.05, depth: 0.45 }, new Vector3(0, 1.36, 0), jacket, this.root);
...
class GameWorld {
  ...
  constructor(scene: Scene, canvas: HTMLCanvasElement) {
    ...
    this.createNpcs();
    this.createMission();
    this.camera = new FreeCamera("follow-camera", new Vector3(10, 8, -12), scene);
  ...
  private createEnvironment() {
    ...
    const road = material(this.scene, "asphalt", palette.road);
  ...
  private createHouse(x: number, z: number, color: Color3, index: number) {
    const base = material(this.scene, `house-${index}`, color);
  ...
  private createBillboard() {
    ...
    logoPlane.position = new Vector3(0, 5.5, 0.22);
  ...
  private updateCamera(dt: number) {
    ...
    const distance = this.state === "driving" ? 11 : 9.2;
    const desired = target.add(new Vector3(Math.sin(angle) * distance, this.state === "driving" ? 7.2 : 6.3, Math.cos(angle) * distance));
